...
public class GrobidRestProcessFiles {
    ...
    public Response processFulltextDocumentToJSON(final InputStream inputStream,
                                          final int consolidateHeader,
                                          final int consolidateCitations,
                                          final boolean includeRawAffiliations,
                                          final boolean includeRawCitations,
        ...
        try {
            ...
            retVal = engine.processFulltextDocumentToJSON(originFile, config);
            ...
            if (GrobidRestUtils.isResultNullOrEmpty(retVal)) {
                ...
                response = Response.status(Response.Status.OK)
                    .entity(retVal)
                    .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_TYPE + "; charset=UTF-8")
                    .build();
            }
        } catch (NoSuchElementException nseExp) {
            LOGGER.error("Could not get an engine from the pool within configured time. Sending service unavailable.");
